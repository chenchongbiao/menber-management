...
# oauth2_scheme()
async def get_current_user(request: Request, token: str = Depends(oauth2_scheme)) -> Admin:
    """
    ...
    try:
        ...
        raise credentials_exception
    admin = await Admin.get(username=username)
    # 使用redis
    # if await request.app.state.redis.get(admin.username) is None:
